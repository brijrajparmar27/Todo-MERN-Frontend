...
const Home = () => {
  ...
  const getRoomName = () => {
    user &&
      setCurrentRoom(
        user.projects.find((each) => {
          return each.id == project;
        })
      );
  };
  ...
  const handleAdd = (e) => {
    ...
    newTodo &&
      newTodo.trim() &&
      createTodo({ todo: newTodo, createdBY: user._id, folderId: project });
    setNewTodo(null);
    e.target.reset();
  };
  ...
  return (
    <div className="home">
      ...
      <div
        className="header"
        style={{
          backgroundImage: `linear-gradient(45deg,#005858a9,#00ff8c91), url(${hills})`,
        }}
      >
          ...
          <div className="todo_list">
            {pending && pending.length > 0 && (
              <div className="pending_todos">
                ...
                {pending.map((each) => {
                  return (
                    <Todo each={each} isCompleted={false} key={each._id} />
                  );
                })}
              </div>
            )}
